...
static inline uint32_t GpuPro(enum gpu_shader_t shader_type, const char * shader_string, int feedback_count, const char ** feedback_names) {
  ...
  {
    int compiled = 0;
    void (*glGetShaderiv)(uint32_t, uint32_t, int *) = SDL_GL_GetProcAddress("glGetShaderiv");
    ...
    if (!compiled) {
      ...
      if (info_len > 1) {
        ...
        void (*glGetShaderInfoLog)(uint32_t, int, int *, char *) = SDL_GL_GetProcAddress("glGetShaderInfoLog");
...
static inline void GpuDebugCallback(uint32_t, uint32_t, uint32_t, uint32_t, int32_t, const char *, void *) {}
